...
Often we want to save our plots outside of R. You can save particular ones using `ggsave()`, or do what we are going to do here, which is write some code to save each of them to a new folder as their own `.svg` (because raster when you can vectorize). I'm linking to [some information](https://www.adobe.com/creativecloud/file-types/image/comparison/raster-vs-vector.html) that provides more discussion about the differences between raster and vector formats.
...
Then, we will save. We are using `pwalk()` here instead of another function like `imap()` because iterates (in this case, applying `ggsave`) but doesn't provide anything in return (which we don't need, since the saving happens by `ggsave`). It is also useful because we could sent a bunch of arguments for our plots (i.e., width, height, dpi) and `pwalk()` takes unlimited arguments. It works on side effects and returns `NULL`.
```{r save to svg, eval = FALSE}
# use pwalk to "walk" across the different plots and save them
# provide the arguments in a list

# to use the path subfolder "img" you need to create that in your working dir
pwalk(
# the args to pass to the function (i.e. ggsave)  
  list(filename = names(my_plots), # what the files should be called
       plot = my_plots, # what plots are we saving
       width = 8, # plot width
       height = 5, # plot height
       units = "in", # width and height units
       path = "img/"), # where should those plots be saved
      ggsave # the function we are aplying.
)
```
