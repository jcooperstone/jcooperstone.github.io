...
You can write in Markdown to code in line line this: There are `` `r '\x60r nrow(happiness)\x60'` `` countries in `happiness` and `` `r '\x60r nrow(life_expectancy)\x60'` `` in `life_expectancy`. There are `` `r '\x60r ncol(happiness)-1\x60'` `` years with data in `happiness` and `` `r '\x60r ncol(life_expectancy)-1\x60'` `` in `life_expectancy`.

And it will render like this: There are `r nrow(happiness)` countries in `happiness` and `r nrow(life_expectancy)` in `life_expectancy`. There are `r ncol(happiness)-1` years with data in `happiness` and `r ncol(life_expectancy)-1` in `life_expectancy`. 
